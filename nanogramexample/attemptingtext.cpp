...
int main(int argc, char** argv)
{
    ...
    else
    {
...

        {
            ...
            auto text = vsg::Text::create();
            text->text = vsg::stringValue::create("horizontalAlignmentwoah\nLEFT_ALIGNMENT");
            text->font = font;
            text->layout = layout;
            text->setup(0, options);
            scenegraph->addChild(text);
...

        {
            ...
            auto text = vsg::Text::create();
            text->text = vsg::stringValue::create("verticalAlignmentwoah\nBOTTOM_ALIGNMENT");
            text->font = font;
            text->layout = layout;
            text->setup(0, options);
            scenegraph->addChild(text);
    ...
    while (viewer->advanceToNextFrame() && (numFrames < 0 || (numFrames--) > 0))
    {
        if (dynamic_text)
        {
            ...
            dynamic_text_label->value() = vsg::make_string("GpuLayoutTechniquewoah: ", viewer->getFrameStamp()->frameCount);
